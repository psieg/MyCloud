...
// Feed the crypto RNG
void crypt_seed(const std::string& data);
void crypt_seed(int64 data);

// Translate Metadata to/from cryptformat
// Encrypts rule, can't be used locally this way!
int crypt_filter_tosrv(mc_sync_ctx *ctx, const std::string& syncname, mc_filter *f);
// Translates rule
int crypt_filter_fromsrv(mc_sync_ctx *ctx, const std::string& syncname, mc_filter *f); //TODO: private because implicit?
// Translate a filter listing in-place
int crypt_filterlist_fromsrv(mc_sync_ctx *ctx, const std::string& syncname, std::list<mc_filter> *l);
// Does not translate size
int crypt_file_tosrv(mc_sync_ctx *ctx, const std::string& path, mc_file *f);
// Also translates size, as locally only the unencrypted size counts, or sizetranslate is used
int crypt_file_fromsrv(mc_sync_ctx *ctx, const std::string& path, mc_file *f); //TODO: private because implicit?
// Translate a directory listing in-place
int crypt_filelist_fromsrv(mc_sync_ctx *ctx, const std::string& path, std::list<mc_file> *l);
// Translate a data-std::string to a keyring-list
int crypt_keyring_fromsrv(std::string data, const std::string& password, std::list<mc_keyringentry> *l);
// Translate from keyring-list to server-format
int crypt_keyring_tosrv(std::list<mc_keyringentry> *l, const std::string& password, std::string *data);

// Filestring differs because of the size translation
void crypt_filestring(mc_sync_ctx *ctx, mc_file *f, std::string *s);

// Calculate MD5 for the file wether crypted or not
int crypt_filemd5_new(mc_crypt_ctx *cctx, unsigned char hash[16], const std::string& fpath, size_t fsize);
int crypt_filemd5_new(mc_crypt_ctx *cctx, unsigned char hash[16], const std::string& fpath, size_t fsize, FILE *fdesc);
int crypt_filemd5_known(mc_crypt_ctx *cctx, mc_file *file, unsigned char hash[16], const std::string& fpath);
int crypt_filemd5_known(mc_crypt_ctx *cctx, mc_file *file, unsigned char hash[16], const std::string& fpath, FILE *fdesc);
...
int crypt_putfile(mc_crypt_ctx *cctx, const std::string& path, mc_file *file, int64 blocksize, FILE *fdesc);
int crypt_addfile(mc_crypt_ctx *cctx, int id, int64 offset, int64 blocksize, FILE *fdesc, unsigned char hash[16]);
int crypt_patchfile(mc_crypt_ctx *cctx, const std::string& path, mc_file *file);
